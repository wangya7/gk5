...
0 <= prices[i] <= 1000
通过次数36,597提交次数119,971
 */
public class LC00188 {

    public static void main(String[] args) {
        // System.out.println(new LC00188().maxProfit(2, new int[]{2, 4, 1}));
        System.out.println(new LC00188().maxProfit(2, new int[]{3, 2, 6, 5, 0, 3}));
        // System.out.println(new LC00188().maxProfit(2, new int[]{9, 8, 7, 5, 4, 3}));
    }
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        for (int i = 1; i <= len; i++) {
            for (int j = 1; j <= k; j++) {
                ...
                // 不持有股票   卖出啦，注意卖出的时候交易次数不变
                dp[i][j][0] = Math.max(dp[i - 1][j][0], dp[i - 1][j][1] + prices[i - 1]);
                System.out.println("dp[" + i + "][" + j + "][1]=" + dp[i][j][1] + ", \t\tdp[" + i + "][" + j + "][0]=" + dp[i][j][0]);
